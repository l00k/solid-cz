...
xdescribe('Staking / Management', async() => {
    ...
    it('Creates proper pools', async() => {
        ...
        // create by owner with require amount
        {
            ...
            expect(rewardPools.rewardsRatio).to.be.equal(tokenFormat(10000).div(month));
            expect(rewardPools.expiresAt).to.be.equal(tx.timestamp + month);
